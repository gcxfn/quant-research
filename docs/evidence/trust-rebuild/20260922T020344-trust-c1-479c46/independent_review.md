...
| R8 | 删失审计与 C0 台账交叉核对 | 解析 censoring_audit.csv | 102 个 B1 事件窗全部 `window_complete=True`（与 C0 结论"B1 无越界"一致）；86 个 B1 事件证券的 2020-12 边界锚点全部 `window_complete=False` 且保留不删；44 只退市证券单独成行。**R1 返工更正（2026-09-22）：原文误写为「B2/A 家族」——该 86 个锚点来自 B1 漏斗证券（censoring_audit.py family=B1，data_cards §7 同）；B2/A 的 2020-12-29 越界接触未在本审计独立复算，仍以 C0 access_ledger AL-01 登记为准** |
