...
- 主计划 C0→C9 的依赖顺序与各验收门：属计划内约定，按文件执行即可。
- 每席 10% 与单只 25% 的语义区分：C0 已在合同快照中分列，不需要用户选择。

### 裁决结果（2026-09-22，用户）

用户裁定"都按建议来"并补充"创业板个股无权限"，12 项全部关闭；完整落实明细见 `docs/decisions/2026-09-22-c0-open-items-adjudication.md`：

1. 未提交工作提交为两段（归因批次 / 信任重建＋裁决轮）；未授权推送远端。
2. 失败运行补登记——已由 C4 完成，无需动作。
3. 不加分支保护、不开独立分支；维持独立会话审阅＋台账（`procedural_only`）。
4. 建立最小 CI（`.github/workflows/tests.yml`，uv＋pytest）；5 个真实数据测试加数据缺失跳过守卫。
5. K 角色由用户承担；接受同机数据现状，靠接触台账约束。
6. 舍入规则——用户已追认（返工 R1）。
7. E0 语义确认为有意设计（×1.01＝签发门槛、限价＝决策锚价）；C6-E1 按预登记另行实现。
8. 创业板——用户确认账户无创业板权限；维持现行排除，决策记录即账户权限证据。
9. 研究范围收窄为 2015–2020 开发期＋2021–2024 单独授权（已并入 AGENTS §3）。
10. AGENTS 六处修订合入（防御资产退出条款按提案保留）；主线自此为个股＋现金。
11. 旧"C0"改称"复审修正批次（C0-attrib）"，README 已同步。
12. 授权起草 C1-S 预登记（草案 `docs/research/exp-20260922-c1s-seat-priority-prereg.md`，未授权运行）。
