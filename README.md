...
**阶段 A/B 复审落地：复审修正批次（C0-attrib）归因修正完成，0/4 存活的判定不变，失败归因已更正。** 尚未找到符合完整策略契约且费用后年化达到 10% 的策略。当前证据链：

- **源码复审（C1–C4/Q1/Q2）三轮修复收官**：训练窗口冻结、预测名单未来依赖、退出计数失真、20 万口径等缺陷全部确认并修复（[审计报告](docs/code_audit_stock_first.md)、[受影响运行台账](docs/evidence/affected_runs.csv)）；
- **统一 50 万基线 v4 成立**（[stock_cash_baseline.md](docs/research/stock_cash_baseline.md)）：FULL-bin 净 CAGR −2.90%/MDD −27.29%；五层拆解中唯一大额正贡献是账户级降仓 +10.69pp（2026-09-22 起撤回可加性读法：IDEAL−FULL 为四因素联合差、残差 −0.64pp 不可识别）；
- **阶段 B 两家族四候选 0/4 存活**（[exp-20260921-event-families.md](docs/research/exp-20260921-event-families.md)）：A1/A2/B1/B2 全部未过"净值为正"与"执行率 ≥90%"两门；随机对照（同结构随机选股，+1.95% 均值）好于全部候选；分年口径已补 2015 年（A1 隐含 +14.05%、A2 +2.60%，"六年全负"撤回）；
- **失败归因（C0-attrib 修正版，2026-09-22）**（[exp-20260921-eventfam-attribution.md](docs/research/exp-20260921-eventfam-attribution.md) §0，运行 `artifacts/runs/20260922T002611-eventfam-attrib2-eed5cf/`）：原"×1.01 限价选错成交样本"的叙事撤回——B1 的 102 事件中 63 个因**席位满员被跳过、从未签发订单**（该组事后 20 日 +10.62% vs 签发组 +1.34%），缺口定位在**席位接纳**而非订单成交（决策锚到成交漂移中位 0.00%）；B1 实为纯数值上修信号（type 分支 0/102），事件级增量在修正口径下保留（+3.66pp 均值对均值 / +5.99pp vs 池中位；102/102 窗口完整落 Dev），但已成交 35 笔从实际成交成本起 +0.50%、净损益 −¥16,252。

连续八轮无候选过门。已验证有效的构件：账户级降仓、分散、执行纪律；随机对照 +2~3%/年为现阶段账户级结构底线。**复审修正批次（C0-attrib）继续条件成立：若重开 B1，方向是 C1-S 臂（席位接纳优先级，预登记草案已起草、待 C5 证据门与用户授权）。** 全仓 804 项测试通过。

**2026-09-22 信任重建收官：阶段 C0–C4 工程验收 accepted（独立审阅＋返工 R1，[主计划](docs/plans/stock-first-trust-rebuild.md)、[验收矩阵](docs/quant_stage_plan_20260922/02_acceptance_matrix.csv)）；用户裁决关闭全部 C0 待决项——主线定为个股＋现金、50 万不可覆盖、2015–2020 开发期、创业板按无权限排除、CI 上线（[决策记录](docs/decisions/2026-09-22-c0-open-items-adjudication.md)）。C5 及之后阶段未开启。**
...
- 已验证运行命令（仓库 `.venv`，Windows）：
  ```powershell
  .venv\Scripts\python.exe -m pytest tests -q                          # 全量测试（703 项，含 90+ 契约/zones 测试）
