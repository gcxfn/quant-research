...
**外部源码复审修复 + 基线重建 + 阶段 B 四候选全部完成，0/4 存活，归因结论已出。** 尚未找到符合完整策略契约且费用后年化达到 10% 的策略。当前证据链：
...
- **统一 50 万基线 v4 成立**（[stock_cash_baseline.md](docs/research/stock_cash_baseline.md)）：FULL-bin 净 CAGR −2.90%/MDD −27.29%；五层拆解中唯一大额正贡献是账户级降仓 +10.69pp，选股信号自身为负；
- **阶段 B 两家族四候选 0/4 存活**（[exp-20260921-event-families.md](docs/research/exp-20260921-event-families.md)）：A1/A2/B1/B2 全部未过"净值为正"与"执行率 ≥90%"两门；随机对照（同结构随机选股，+1.95% 均值）好于全部候选；
- **失败归因**（[exp-20260921-eventfam-attribution.md](docs/research/exp-20260921-eventfam-attribution.md)）：收益死在**成交选择**——×1.01 追价限价单系统性只成交失败回落批（选择差 −2.5~−6.0pp）；A 家族相对同资格随机对照增量为负（判死）；**B1 业绩上修事件级 +7.07%/20 日（+6.0pp vs 池中位，全项目迄今最大信号级增量），漂移一半发生在第 11–20 日——合法窗口存在、被订单结构放弃**，重开须新预登记换入场结构。

连续八轮无候选过门。已验证有效的构件：账户级降仓、分散、执行纪律；随机对照 +2~3%/年为现阶段账户级结构底线。**方向待用户定：B1 入场结构重开预登记，或转向暴露/风险预算结构、新数据源。** 全仓 703 项测试通过。
