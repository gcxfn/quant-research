...
Prereg (frozen): docs/research/exp-20260921-etf-momentum-simple-prereg.md
...
def _fail(reason: str) -> None:
    ...
    (RUN_DIR / "manifest.json").write_text(json.dumps({
        "run_id": RUN_DIR.name,
        "experiment_id": "exp-20260921-etf-momentum-simple",
...
def provider(day: date, sess: str, ledger: dict) -> list[dict] | None:
    ...
    if T is not None:
        ...
        # SIMPLE FORM (prereg exp-20260921-etf-momentum-simple): no account
        # overlay -- the absolute-momentum gate is the only bear protection.
        targets = {s: ATTACK_BUDGET / TOP_N for s in picks}
...
log(f"  engine: {time.perf_counter() - t_c:.1f}s; overlay: none (simple); "
...
metricsB = {"net_cagr": net, "max_drawdown": mdd,
            ...
            "b3mix_mean": b3p,
            "overlay": "none (simple form: absolute-momentum gate only)",
...
manifest = {
    ...
    "trial_accounting": {"strategy_line": {
        "before": 291, "this_round": 0 if SMOKE else 1,
        "after": 291 if SMOKE else 292}},
