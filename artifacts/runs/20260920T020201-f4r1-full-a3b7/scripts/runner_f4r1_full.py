# -*- coding: utf-8 -*-
"""exp-20260919-zone-sheet-f4r1 FULL-DEV runner -- engine v1.4 (pin 7ad35014,
zones mode), prereg FROZEN (docs/research/exp-20260919-zone-sheet-f4r1-prereg.md
+ configs/experiments/f4r1-zone-sheet.json).

Copy-adapted from the ACCEPTED smoke runner
(artifacts/runs/20260920T011431-f4r1-smoke-4d9e/scripts/runner_f4r1.py):
zones frame construction, the three arms, the A0 anchor and the data pins
are carried verbatim; ONLY the window (smoke W1/W2 -> full dev
2015-01-05..2020-12-31) and the gate layer changed.

Gates (frozen prereg sec 2; zero search):
- gates 1-7 carried VERBATIM from the F3R3 runner gate code
  (artifacts/runs/20260919T201500-f3r3-industry-cap-4b2e/scripts/
  runner_f3r3.py, sec 13); anchors B1(m) = R16 C05 T200-40 rate-only
  (net +0.60%, maxDD -39.82%, loaded from R16 metrics.json configs.C05
  verbatim and asserted against the frozen 4-dp values), B3' = -0.0199.
...
Stop rules (prereg sec 7): A0 anchor byte mismatch -> STOP before the arms
(no trial consumed).  Any arm dev_pass AND >= 10% -> record and stop
extending (the remaining arms still run to completion in this single
process; val requires explicit user approval).  All three arms eliminated
-> attribution report, no F4R2.

Trial accounting: strategy line 270 -> 273 booked at THIS run (three arm
trials; registered in the main-conversation ledger).  Factor line FT01=120
unchanged.
...
SMOKE_RUN = ROOT / "artifacts/runs/20260920T011431-f4r1-smoke-4d9e"
...
# F4R1 frozen sleeve parameters (prereg sec 3/4/5; zero grid; smoke verbatim)
SLEEVE_K = 10
...
EXPECTED = {
    ...
    "smoke_manifest": (None, SMOKE_RUN / "manifest.json"),
...
log("== 2. exposures (T200-40 month-end, full dev) ==")
...
def build_zones_frame(arm: str) -> tuple[pl.DataFrame, dict]:
    """Per-arm full-dev zones frame (frozen prereg sec 4/5; smoke verbatim)."""
    rows: list[dict] = []
...
# (smoke runner verbatim; the anchor is the FULL-DEV A0, already byte-equal
# at the smoke run -- it gates the arms per prereg sec 7, no trial consumed)
...
log("== 10. arm runs: 3 arms x full dev ==")
...
# === 13. stop-rule verdict + disclosures =====================================
log("== 13. stop-rule verdict + disclosures ==")
...
if winner:
    STOP_NOTE = (f"{len(winner)}/{len(ARMS)} arm(s) dev_pass AND dev net "
                 f"CAGR >= 10% ({', '.join(winner)}): STOP per prereg sec 7; "
                 "present to user; val (2021-2024) consumption requires "
                 "explicit user approval.  The remaining arms completed in "
                 "this single process but nothing further is extended.")
elif n_pass:
    STOP_NOTE = (f"{n_pass}/{len(ARMS)} arms pass all dev gates but none "
                 "reaches dev net CAGR >= 10%: goal NOT met; present to "
                 "user; no extension beyond F4R1 without a new decision.")
else:
    STOP_NOTE = (f"no ARM passed the dev gates ({n_pass}/{len(ARMS)}): "
                 "all three eliminated -> attribution report; STOP per "
                 "prereg sec 7; F4R2 NOT opened autonomously.")
log("  " + STOP_NOTE)
...
DISCLOSURES = [
    ...
    "F4R1-S2 接缝（预登记 §11 裁定接受）：臂 B 的月末减仓原语在 zones 模式"
    "结构性缺失（契约 BR-7 非目标、S-v14-15 禁止个股入意图帧）；臂 B 实际"
    "保留 = w_T=e_T×0.06/0.90 缩放 + 月边界续持/退出再平衡，按\"部分路径"
    "对照\"解读；A vs B 差异若成为结论关键点须先复核本接缝。",
...
mg = {
    "run_id": RUN_DIR.name,
    "experiment_id": "exp-20260919-zone-sheet-f4r1",
    "scope": "FULL DEV 2015-01-05..2020-12-31, three arms (F4R1-A/B/C) + "
             "F4R1-A0 anchor; eight frozen gates (prereg sec 2)",
    ...
    "trial_accounting": {
        "strategy_line": "270 -> 273 booked at THIS run (three arm trials; "
                         "F4R1-S1/S2 seams adjudicated prereg sec 11)",
        "registered_in": "main-conversation trial ledger",
        "factor_line_FT01": 120,
        "anchor_trials": "F4R1-A0 anchor consumed no trial (prereg sec 7)"},
    ...
    "f4r1_a0_anchor": {**{k: v for k, v in A0_CMP.items()},
                       "rule": "filecmp byte comparison vs F3R3-A0 outputs; "
                               "mismatch = STOP before arms (prereg sec 7)"},
}
...
# ---- report ---------------------------------------------------------------
lines = ["# F4R1 全期运行报告（exp-20260919-zone-sheet-f4r1，dev 2015–2020）", "",
...
lines += ["", "## 三、死因归因（淘汰臂；预登记 §7 重点门 3/4/5/8）", ""]
for arm in ARMS:
...
lines += ["## 四、停止规则结论", "", f"- {STOP_NOTE}", "",
          "与 F3R3（ICW 净 +4.88% / EW +3.42%，均淘汰）和 R3-06（+11.06%，"
          "v3 dev_pass）对照：F4R1 三臂净 CAGR = "
...
lines += ["", "## 六、运行身份与预算", "",
          ...
          "- 试验计账：策略线 270 → 273（三臂，本次全期运行入账，主对话"
          "登记台账）；A0 锚不耗试验；因子线 FT01=120 不变。",
          "- runner 核对口径（沿冒烟）：逐月首扫准入重放（2015-01）、"
          "行业受阻重建、fills 路由/阶梯/止盈/止损价逐分核对、现金守恒与"
          "权益恒等式、zones 统计自洽、门 8 结构与台账。"]
...
manifest = {
    ...
    "scope": "FULL DEV 2015-01-05..2020-12-31, three arms + F4R1-A0 anchor; "
             "eight frozen gates; prereg sec 2/7/9/10/11 applied verbatim",
    "started_at_utc": datetime.fromtimestamp(T0, tz=timezone.utc).isoformat(),
    "ended_at_utc": datetime.now(timezone.utc).isoformat(),
    "wall_seconds": time.perf_counter() - T0,
    "peak_rss_gb": PEAK_RSS,
    "command": [sys.executable,
                f"artifacts/runs/{RUN_DIR.name}/scripts/runner_f4r1_full.py"],
    "random_seed": None,
    ...
    "runner_adaptations_disclosed": {
        "leg_intent_priority_999": "F4R1-S1 (prereg sec 11 adjudicated): "
                                   "BR-1 guard requires intent priority < "
                                   "zone_priority_base(1000); the ETF leg "
                                   "is funded BEFORE stock ladders in "
                                   "same-decision cash competition.",
        "arm_B_path_wt_only": "F4R1-S2 (prereg sec 11 adjudicated): zones "
                              "mode has no month-end reduce-to-target "
                              "primitive (BR-7) and stock symbols cannot "
                              "appear in the intent frame (S-v14-15); arm B "
                              "keeps the T200-40 path ONLY through w_T = "
                              "e_T*0.06/0.90 scaling; interpreted as a "
                              "partial-path comparator.",
        ...
        "smoke_provenance": "runner copied from the accepted smoke run "
                            + SMOKE_RUN.name + "; window and gate layer "
                            "changed only."},
    "halfday_identity_correction": "prereg sec 10: effective identity = "
                                   "manifest.json sha256_16 2a414174b5df2eee"
                                   " + 18,486,939 rows (registered above)",
    "outputs": outs,
}
