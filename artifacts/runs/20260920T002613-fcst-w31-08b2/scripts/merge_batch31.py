#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-31 专用：把 6 段逐行结构化判断(batch_031_judge_partN.jsonl)与 row_index 元信息
程序化合并为 batch_031_partN.jsonl（全字段），并做合并前预校验：
...
沿自 wave-25 merge_batch25.py，仅改 batch=25→31、ordinal 基 14400→18000、文件名 batch_025→batch_031，校验逻辑逐行保留（含冻结区断言）；批次过滤行由主对话显式替换（sed 模式对代码行无效的教训，见 w26 fix-note）。

用法: python merge_batch31.py --repo-root D:/量化
产物: work/batch_031_part1..6.jsonl、data/features/fcst-reason-struct-full-20260918/batch_031.jsonl
"""
import argparse
import csv
import glob
...
def main():
    ...
    for part in range(1, 7):
        jpath = os.path.join(work, f"batch_031_judge_part{part}.jsonl")
        with open(jpath, encoding="utf-8") as f:
            judges = [json.loads(x) for x in f.read().splitlines() if x.strip()]
        expect_lo = 18000 + (part - 1) * 100
        ...
        with open(os.path.join(work, f"batch_031_part{part}.jsonl"), "w",
    ...
    if fails:
        ...
        for x in fails:
            print("FAIL:", x)
        raise SystemExit(1)
    out = os.path.join(features_dir, "batch_031.jsonl")
    with open(out, "w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(merged) + "\n")
    print(json.dumps({
        "merged_rows": len(merged),
        "part_files": [f"batch_031_part{i}.jsonl" for i in range(1, 7)],
