...
- Fees: ETF legs pay no stamp (contract section 8.3 precedence over any
  meta stamp_sell value); commission max(rate x notional, min) honors
  per-symbol overrides; stamp_buy charges on buys only when explicitly
  overridden (default 0 = v1.1).
...
def _tick_round(price: float, tick: float) -> float:
    ...
    0.001-tick ETF anchor construction).  Half-up, not banker's rounding."""
    scaled = price / tick
    return math.floor(scaled + 0.5) * tick
...
def run_band_backtest(
    ...
    *,
    ...
    validated for BOTH entry points; the section 8.2 ETF session ROUTING is
    an intent-layer feature (ETF legs have no 11:30 decision point), so the
    documented surface for the ETF leg is
    ...
    if intent_engine:
        ...
        for it in INTENTS:
            if it["symbol"] in etf_meta and it["first_s"] == "am":
                raise BandContractError(
    ...
    def limits_at(sym: str, day: date) -> tuple[float | None, float | None]:
        ...
        if row_f is not None:
            # v1.2 section 8.3 (TBD-A): stk_limit has zero fund coverage ->
            # ETF limits are COMPUTED: half-up(preclose x (1 +/- band)) on
            # the 0.001 tick; stk_limit rows for ETF symbols are ignored.
            pc = preclose_at(sym, day)
            if pc is None:
                return (None, None)
            band_f = row_f["band"]
            return (_tick_round(pc * (1.0 + band_f), ETF_PRICE_TICK),
                    _tick_round(pc * (1.0 - band_f), ETF_PRICE_TICK))
        lim = limits.get(sym)
        if lim is None:
            return None, None
    ...
    def eff_stamp_buy(sym: str, notional: float) -> float:
        """v1.2: buy-side stamp ONLY when a meta override supplies it
        (A-share default 0 -- v1.1 behavior is untouched)."""
        row_f = meta_rows.get(sym)
        if row_f is not None and "stamp_buy" in row_f:
    ...
    def full_quantity_affordable(sym: str, price: float, shares: int,
        ...
        cost = shares * price
        return cost + eff_commission(sym, cost) <= available + _FLOAT_TOL
    ...
    def process_session(day: date, sess: str) -> None:
        ...
        for order in buys:
            ...
            if not full_quantity_affordable(order.symbol, order.anchor_price,
                                            want, available):
            ...
            fee = eff_commission(order.symbol, cost)
        ...
        for sym in sorted(s for s, st in risk_state.items() if st["armed"]):
            if sym in etf_meta and sess == "am":
                ...
                # host supplies am rows for an ETF)
                continue
            st = risk_state[sym]
    ...
    stats = {
        ...
        "engine_version": "band_engine v1.2",
        "etf_leg": {
            ...
            "routing": ("section 8.2: ETF pm decisions live next pm; ETF "
