...
OUT = _OUT_ROOT / f"factor_{_WINDOW}" / "D_fund"
...
def pit_checked(raw, keep, name):
    ...
    # The validation source extends the frozen development source with later
    # revisions.  Preserve the key-set check; do not reject those legitimate
    # post-dev rows solely because the dev port-count differs.
    if _WINDOW == "dev":
...
fin_v = fin.filter(pl.col("signal_date") <= WINDOW_END).sort("symbol", "signal_date", "end_date")
rows = []
for t in ME:
...
db = pl.read_parquet(
    _OUT_ROOT / f"factor_{_WINDOW}" / "B_value" / "B04.parquet")
...
# 披露三件套量化
pit_cov = sig_cap(asof, date(2020, 11, 30)).group_by(
