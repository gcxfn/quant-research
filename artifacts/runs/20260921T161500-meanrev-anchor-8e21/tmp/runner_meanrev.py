# -*- coding: utf-8 -*-
"""Anchored mean-reversion (stable-company overshoot repair) - frozen prereg.

Prereg: docs/research/exp-20260921-mean-reversion-prereg.md
...
PREREG = ROOT / "docs/research/exp-20260921-mean-reversion-prereg.md"
...
DEV_START, DEV_END = date(2016, 12, 31), date(2020, 12, 31)
...
for c in ("M1", "M2"):
    ...
    g = {"1_cagr_ge_B0_plus_2pp":
         ...
         "4_turnover_le_6": a["avg_annual_turnover"] <= 6.0}
    ...
    log(f"  {c}: cagr {a['cagr']*100:+.2f}% mdd {a['mdd']*100:.1f}% "
        ...
        f"{'PASS' if g['dev_pass'] else 'fail ' + str(g['failed'])}")
...
if not SMOKE:
    rep = ["# 估值带锚定均值回归（dev 信号级）报告", "",
           f"- 运行 `{RUN_DIR.name}`；预登记冻结；2017-01..2020-12；"
           "月度合成口径（月末收盘、close/preclose 链收益）。", "",
           "| 曲线 | 年化 | 回撤 | 优势年 | 年换手 | 判定 |",
           "|---|---|---|---|---|---|"]
    for c in results:
        a = metrics[c]
        verdict = (gates[c]["dev_pass"] if c in gates else "-")
...
manifest = {
    "run_id": RUN_DIR.name,
    "experiment_id": "exp-20260921-mean-reversion",
    ...
    "trial_accounting": {"strategy_line": {"before": 292,
                                           "this_round": 0 if SMOKE else 6,
                                           "after": 292 if SMOKE else 298}},
