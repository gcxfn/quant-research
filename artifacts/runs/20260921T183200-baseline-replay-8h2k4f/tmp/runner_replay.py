        ...
        if T is not None:
            ...
            for r in overlay_rows:
                r.setdefault("exit_reason",
                             "account_trim" if r["side"] == "sell" else None)
...
def _tally_reasons(reasons: list[dict]) -> dict:
    """Provider-side TRIGGER counts by exit_reason (audit C3)."""
    out: dict[str, int] = {}
    for r in reasons:
...
def _tally_filled_reasons(reasons: list[dict], fills) -> dict:
    """FILLED counts attributed to the provider-side reason via
    (decision_date, decision_session, symbol) -- triggers vs completions
    are reported separately (audit C3 requirement)."""
    key = {(r["day"], r["sess"], r["symbol"]): r["reason"] for r in reasons}
    out: dict[str, int] = {}
    if fills.height:
        for r in fills.select("symbol", "side", "decision_date",
            ...
            if r["side"] != "sell":
                continue
            k = (r["decision_date"].isoformat(), r["decision_session"],
                 r["symbol"])
            rsn = key.get(k)
            if rsn is not None:
...
for name, *_ in CURVES:
    ...
    metrics[name] = {
        "net_cagr": net, "max_drawdown": mdd_from_equity(sv),
        ...
        "reasons_filled": _tally_filled_reasons(
            results[name]["pack"]["reasons"], res.fills)}
    log(f"  {name}: net {net*100:+.2f}% mdd {metrics[name]['max_drawdown']*100:.2f}% "
        f"fees {metrics[name]['total_fees']:.0f} fills {res.fills.height} "
        f"reasons {metrics[name]['reasons_provider']}")
