#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-12 标注辅助：生成"ordinal|type|REASON"单行紧凑视图（控制标注上下文预算）。

沿自 wave-11 extract_cues.py，两处适配：
- batch 默认 10→12，产物 work/batch_012_cues.txt；
- batch 12 的 type 不再恒定（略增 410 / 略减 190，row_index 程序确认），
  行前缀加入 type 字段供 supports_direction 判读，格式为 ordinal|type|REASON。
...
用法: python extract_cues.py --repo-root D:/量化 [--batch 12]
产物: work/batch_012_cues.txt
"""
import argparse
import csv
import glob
