#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-21 标注辅助：生成"ordinal|REASON"单行紧凑视图（控制标注上下文预算）。

沿自 wave-20 extract_cues.py，仅改 batch 默认 20→21，产物 work/batch_021_cues.txt；
行前缀查 row_index 后本批 ordinals 12000–12599 共 600 行 type 分布为
600 行全部续亏（单一 type），故沿用批 14-18/20 的"ordinal|REASON"简化口径
（不保留 type 字段）；supports_direction 按 恶化组=续亏 判定
（利空向原因 +1、利好向原因 -1、中性/基数 0）。
...
用法: python extract_cues.py --repo-root D:/量化 [--batch 21]
产物: work/batch_021_cues.txt
"""
import argparse
import csv
import glob
...
def main():
    ...
    ap.add_argument("--batch", type=int, default=21)
    args = ap.parse_args()
