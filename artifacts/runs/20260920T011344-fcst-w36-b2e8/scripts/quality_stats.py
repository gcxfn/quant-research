...
沿自 wave-12 quality_stats.py，唯一改动：batch_input_chars 读取
work/batch_NNN_cues.txt（wave-13 输入为 extract_cues 紧凑视图，与批 11 口径相同），
不再读取 batch_NNN_input.txt。

用法: python quality_stats.py --repo-root D:/量化 --batch 13 [--fix-note "..."]
...
def main():
    ...
    inp = os.path.join(root, "artifacts", "runs",
                       ...
                       "work", f"batch_{args.batch:03d}_cues.txt")
    ...
    print(json.dumps({
        ...
        "batch_input_chars": batch_entry["batch_input_chars"],
