#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-39 专用：把 6 段逐行结构化判断(batch_039_judge_partN.jsonl)与 row_index 元信息
程序化合并为 batch_039_partN.jsonl（全字段），并做合并前预校验：
...
文件名 batch_024→batch_039，校验逻辑逐行保留（含冻结区断言）。

用法: python merge_batch28.py --repo-root D:/量化
产物: work/batch_039_part1..6.jsonl、data/features/fcst-reason-struct-full-20260918/batch_039.jsonl
"""
import argparse
import csv
import glob
...
def main():
    ...
    batch = idx.filter(pl.col("batch_id") == 39).sort("batch_idx")
    ...
    for part in range(1, 7):
        jpath = os.path.join(work, f"batch_039_judge_part{part}.jsonl")
        with open(jpath, encoding="utf-8") as f:
            judges = [json.loads(x) for x in f.read().splitlines() if x.strip()]
        expect_lo = 22800 + (part - 1) * 100
        ...
        with open(os.path.join(work, f"batch_039_part{part}.jsonl"), "w",
    ...
    if fails:
        ...
        for x in fails:
            print("FAIL:", x)
        raise SystemExit(1)
    out = os.path.join(features_dir, "batch_039.jsonl")
    with open(out, "w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(merged) + "\n")
    print(json.dumps({
        "merged_rows": len(merged),
        "part_files": [f"batch_039_part{i}.jsonl" for i in range(1, 7)],
