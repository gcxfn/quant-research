#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-29 标注辅助：生成"ordinal|REASON"单行紧凑视图（控制标注上下文预算）。

沿自 wave-24 extract_cues.py，仅改 batch 默认 24→25，产物 work/batch_029_cues.txt；
行前缀查 row_index 后本批 ordinals 14400–14999 共 600 行 type 分布为
...
用法: python extract_cues.py --repo-root D:/量化 [--batch 29]
产物: work/batch_029_cues.txt
"""
import argparse
import csv
import glob
...
def main():
    ...
    ap.add_argument("--batch", type=int, default=29)
    args = ap.parse_args()
