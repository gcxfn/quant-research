#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-9 专用：把 6 段逐行结构化判断(batch_009_judge_partN.jsonl)与 row_index 元信息
程序化合并为 batch_009_partN.jsonl（全字段），并做合并前预校验：
- ordinal 连续且与 row_index batch_id=9 的 batch_idx 顺序一一对应
- 元信息 ts_code/end_date/ann_date/update_flag 直接取自 row_index（与渲染输入同源，零手抄）
- key_quote 必须是 raw change_reason 的逐字子串（与 check_batch_full 同口径预检）

沿自 wave-8 merge_batch8.py（sha256 ba3f19b5903c8accc733a91818074cf43cc3348c3b40222d42088fdaaf6ea28f），
仅改 batch=8→9、ordinal 基 4200→4800、文件名 batch_008→batch_009，校验逻辑逐行保留。

用法: python merge_batch9.py --repo-root D:/量化
产物: work/batch_009_part1..6.jsonl、data/features/fcst-reason-struct-full-20260918/batch_009.jsonl
"""
import argparse
import csv
import glob
...
def main():
    ...
    batch = idx.filter(pl.col("batch_id") == 9).sort("batch_idx")
    ...
    for part in range(1, 7):
        jpath = os.path.join(work, f"batch_009_judge_part{part}.jsonl")
        with open(jpath, encoding="utf-8") as f:
            judges = [json.loads(x) for x in f.read().splitlines() if x.strip()]
        expect_lo = 4800 + (part - 1) * 100
        ...
        with open(os.path.join(work, f"batch_009_part{part}.jsonl"), "w",
    ...
    if fails:
        ...
        for x in fails:
            print("FAIL:", x)
        raise SystemExit(1)
    out = os.path.join(features_dir, "batch_009.jsonl")
    with open(out, "w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(merged) + "\n")
    print(json.dumps({
        "merged_rows": len(merged),
        "part_files": [f"batch_009_part{i}.jsonl" for i in range(1, 7)],
        "output": os.path.relpath(out, root).replace("\\", "/"),
        "precheck": "OK (600/600, quotes verbatim, vocab closed, secondary!=primary)",
