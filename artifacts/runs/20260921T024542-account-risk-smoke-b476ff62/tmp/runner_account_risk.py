# -*- coding: utf-8 -*-
"""Account-level risk overlay on the dynamic half-day replay (F3-ICW carrier).
...
K = 10
# The registered full run of this experiment (…-account-risk-full-8f16a4e9)
# ran on the engine identity bcbdd44bb855b803 and its manifest pins that hash.
# After it finished (2026-09-21T03:05Z) another task added the v1.5
# ``etf_routing='pm_only'`` option to the same file; the default ``'paired'``
# routing leaves every v1.4.2 path byte-identical.  This SMOKE therefore pins
# the current identity and exists only as an engineering re-run of the host /
# provider path -- it produces no conclusion and is not the registered
# comparison run.
...
def make_provider(arm: str, config_id: str, marks: LedgerMarks):
    ...
    host = DynamicOverlayHost(config_id, marks)
