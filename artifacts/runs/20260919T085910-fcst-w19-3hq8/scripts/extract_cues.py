#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-19 标注辅助：生成"ordinal|type|REASON"单行紧凑视图（控制标注上下文预算）。

沿自 wave-18 extract_cues.py，仅改 batch 默认 18→19，产物 work/batch_019_cues.txt；
行前缀查 row_index 后恢复批 10-13 的"ordinal|type|REASON"格式（批 19 区段
ordinals 10800–11399 共 600 行 type 分布为 略增 355 / 续亏 245，type 混合，
supports_direction 须按 type 所属组（略增=改善组、续亏=恶化组）判定，故必须
在视图中保留 type 字段，不沿用批 14-18 的"ordinal|REASON"简化口径）。
...
- 元信息（ts_code/end_date/ann_date/update_flag）不进视图：merge 脚本从
  row_index 程序化合入；type 已进视图（本批混合，见上）。

用法: python extract_cues.py --repo-root D:/量化 [--batch 19]
产物: work/batch_019_cues.txt
"""
import argparse
import csv
import glob
