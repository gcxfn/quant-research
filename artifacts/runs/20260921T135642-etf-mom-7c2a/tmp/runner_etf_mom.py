# -*- coding: utf-8 -*-
"""ETF momentum rotation (sector ETFs included) - frozen prereg run.

Prereg (frozen): docs/research/exp-20260921-etf-momentum-prereg.md
...
def _fail(reason: str) -> None:
    ...
    (RUN_DIR / "manifest.json").write_text(json.dumps({
        "run_id": RUN_DIR.name,
        "experiment_id": "exp-20260921-etf-momentum",
...
stageA_out.write_text(json.dumps({"metrics": stageA, "gates": gatesA,
                      ...
                      encoding="utf-8")
if not gatesA["pass_all"] and not (SMOKE and os.environ.get("FORCE_STAGE_B")):
    log("stage A failed gates; stage B not executed (prereg order)")
    if not SMOKE:
        (RUN_DIR / "outputs" / "metrics_and_gates.json").write_text(
            json.dumps({"stageA": {"metrics": stageA, "gates": gatesA},
                        "stageB": None}, ensure_ascii=False, indent=1,
                       default=str), encoding="utf-8")
        rep = ["# ETF 动量轮动（含板块 ETF）dev 报告（止于阶段 A）", "",
               f"- 运行 `{RUN_DIR.name}`；预登记已冻结；dev 2015-2020。",
               f"- 阶段 A：净年化 {strat_cagr*100:+.2f}%、回撤 "
               f"{strat_mdd*100:.2f}%（{stageA['period']}）；全池等权 "
               f"{ew_cagr*100:+.2f}%、300 {b300_cagr*100:+.2f}%。",
               f"- 失败门：{[k_ for k_, v in gatesA.items() if not v and k_ != 'pass_all']}。",
        ...
        manifest = {
            ...
            "command": [sys.executable, "artifacts/runs/" + RUN_DIR.name
                        + "/tmp/runner_etf_mom.py"],
            "engine": {"version": "v1.5+clip-price",
                       "clock": "halfday+pm_only"},
            "pins": pins, "pool": list(POOL),
            "trial_accounting": {"strategy_line": {"before": 287,
                                                   "this_round": 1,
                                                   "after": 288}},
            "stage": "A_only_eliminated",
...
def provider(day: date, sess: str, ledger: dict) -> list[dict] | None:
    ...
    if T is not None:
        ...
        pos_val = {p["symbol"]: p["shares"] * p.get("price", 0.0)
...
# max single-name weight from fills + closes
...
# execution rate: seats (symbol, side, source) filled at a point the
# source already governed
filled = set()
...
log(f"  B: net {net*100:+.2f}% exc {net - b1mix['cagr']:.2%} adv {len(adv)}/6 "
    f"mdd {mdd*100:.2f}% turn {max_turn:.2f} w {wmax*100:.1f}% "
    f"exec {exec_rate*100:.1f}% -> "
...
rep = ["# ETF 动量轮动（含板块 ETF）dev 报告", "",
       f"- 运行 `{RUN_DIR.name}`；预登记已冻结；dev 2015-2020。",
       f"- 阶段 A：净年化 {strat_cagr*100:+.2f}%、回撤 {strat_mdd*100:.2f}%、"
       f"全池等权 {ew_cagr*100:+.2f}%、300 {b300_cagr*100:+.2f}%，"
       f"判据 {'PASS' if gatesA['pass_all'] else 'FAIL'}。", "",
       "| 曲线 | 净CAGR | 超额B1mix | 优势年 | 回撤 | 换手 | 单票max | 执行率 | 过/8 | ≥10% | 判定 |",
       "|---|---|---|---|---|---|---|---|---|---|---|",
       f"| MoM-D3 | {net*100:+.2f}% | {(net - b1mix['cagr'])*100:+.2f}pp "
       f"| {len(adv)}/6 | {mdd*100:.2f}% | {max_turn:.2f} | {wmax*100:.1f}% "
       f"| {exec_rate*100:.1f}% | {8 - len(g8['failed_gates'])} "
       f"| {'MET' if g8['goal_cagr_ge_10pct'] else 'no'} "
       f"| {'**PASS**' if g8['dev_pass'] else 'eliminated'} |"]
n_pass = 1 if g8["dev_pass"] else 0
rep += ["", f"## 阶段 B {n_pass}/1 过八门；"
...
manifest = {
    ...
    "engine": {"version": "v1.5+clip-price", "clock": "halfday+pm_only"},
    "pins": pins,
    "pool": list(POOL),
    "trial_accounting": {"strategy_line": {
        "before": 287,
        "this_round": 1 + (0 if SMOKE else 0) + (1 if not SMOKE else 0),
        "after": 287 + (2 if not SMOKE else 0)}},
