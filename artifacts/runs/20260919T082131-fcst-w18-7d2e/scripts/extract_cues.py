#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-18 标注辅助：生成"ordinal|REASON"单行紧凑视图（控制标注上下文预算）。

沿自 wave-17 extract_cues.py，仅改 batch 默认 17→18，产物 work/batch_018_cues.txt；
行前缀查 row_index 后沿用批 14/15/16/17 口径简化为"ordinal|REASON"（批 18 区段
ordinals 10200–10799 共 600 行 type 分布 600 行全部为"略增"，字段冗余，故不含
type；supports_direction 按改善组=略增判定）。
...
- 元信息（ts_code/end_date/ann_date/update_flag）不进视图：merge 脚本从
  row_index 程序化合入；type 以运行输出 type_counts 核定为准。

用法: python extract_cues.py --repo-root D:/量化 [--batch 18]
产物: work/batch_018_cues.txt
"""
import argparse
import csv
import glob
