#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-17 标注辅助：生成"ordinal|REASON"单行紧凑视图（控制标注上下文预算）。

沿自 wave-16 extract_cues.py，仅改 batch 默认 16→17，产物 work/batch_017_cues.txt；
行前缀查 row_index 后沿用批 14/15/16 口径简化为"ordinal|REASON"（批 17 区段
type 分布 600 行全部为"略增"，字段冗余，故不含 type；supports_direction 按
改善组=略增判定）。
...
- 元信息（ts_code/end_date/ann_date/update_flag）不进视图：merge 脚本从
  row_index 程序化合入；type 已核定为全批略增（见上）。

用法: python extract_cues.py --repo-root D:/量化 [--batch 17]
产物: work/batch_017_cues.txt
"""
import argparse
import csv
import glob
...
def main():
    ...
    ap.add_argument("--batch", type=int, default=17)
    args = ap.parse_args()
