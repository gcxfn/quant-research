...
def main():
    ...
    for part in range(1, 7):
        jpath = os.path.join(work, f"batch_034_judge_part{part}.jsonl")
        with open(jpath, encoding="utf-8") as f:
            judges = [json.loads(x) for x in f.read().splitlines() if x.strip()]
        expect_lo = 19800 + (part - 1) * 100
        ...
        with open(os.path.join(work, f"batch_034_part{part}.jsonl"), "w",
    ...
    if fails:
        ...
        for x in fails:
            print("FAIL:", x)
        raise SystemExit(1)
    out = os.path.join(features_dir, "batch_034.jsonl")
    with open(out, "w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(merged) + "\n")
    print(json.dumps({
        "merged_rows": len(merged),
        "part_files": [f"batch_034_part{i}.jsonl" for i in range(1, 7)],
