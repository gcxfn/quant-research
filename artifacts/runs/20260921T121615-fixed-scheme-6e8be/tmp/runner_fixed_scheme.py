...
GCFGS = ["G0", "G1", "G2", "G3"]
CURVES = (["MF-G0"] if SMOKE else
          [f"{a}-{g}" for a in ARMS for g in GCFGS])
...
def _fail(reason: str) -> None:
    ...
    (RUN_DIR / "manifest.json").write_text(json.dumps({
        "run_id": RUN_DIR.name,
        "experiment_id": "exp-20260921-fixed-scheme",
...
for t in sig_days:
    ranked_leg["MF"][t] = sorted(
        eligible[t], key=lambda s: (-composite[t].get(s, -9e9), s))
    ranked_leg["MC"][t] = list(pools_at[t]["ranked"])
for leg in ("MF", "MC"):
    prev: list[str] = []
    for t in sig_days:
...
def make_provider(leg: str, gcfg: str):
    ...
    snr = SingleNameRules(lambda s, d: atr_tbl.atr(s, d))
