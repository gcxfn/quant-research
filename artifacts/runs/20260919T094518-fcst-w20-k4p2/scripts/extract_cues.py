#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-20 标注辅助：生成"ordinal|REASON"单行紧凑视图（控制标注上下文预算）。

沿自 wave-19 extract_cues.py，仅改 batch 默认 19→20，产物 work/batch_020_cues.txt；
行前缀查 row_index 后本批 ordinals 11400–11999 共 600 行 type 分布为
600 行全部续亏（单一 type），故沿用批 14-18 的"ordinal|REASON"简化口径
...
用法: python extract_cues.py --repo-root D:/量化 [--batch 20]
产物: work/batch_020_cues.txt
"""
import argparse
import csv
import glob
