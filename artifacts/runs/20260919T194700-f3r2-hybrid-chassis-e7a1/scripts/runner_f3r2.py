# -*- coding: utf-8 -*-
"""exp-20260919-factor-round-f3r2 runner -- engine v1.3 (pin 84a2443a,
corporate_actions=None = v1.2-identical path), prereg FROZEN.

Copy-adapted from the hybrid family v3 runner
(artifacts/runs/20260919T110500-hybrid-v3-r3ld/scripts/runner_hybrid_v3.py)
per the F3R2 prereg (docs/research/exp-20260919-factor-round-f3r2-prereg.md +
configs/experiments/f3r2-hybrid-chassis.json).  Chassis clauses (universe,
band table, dividend rule, runner assertions, fees, window, Design B leg
semantics, gates 1-7 R16 + gate 8 v3, B1/B3' family anchors) inherited
verbatim from the v3 prereg.  F3R2 differences only:
- F3R2-A0 = R3-06 verbatim replica and regression anchor: 5-frame polars
  .equals() vs the v3 run's R3-06 outputs (intents/fills/events/daily/
  clips_final) + net_cagr match; mismatch aborts before other configs;
- F3R2-EW / F3R2-ICW: the R3-06 rotation sleeve is REPLACED by the F3
  composite stock sleeve (F3R1 frozen outputs verbatim): buffer membership
  (enter top-3, stay while rank<=6, K=3, no q5 filter), weights
  w_T = e_T * (0.20/0.90) with e_T = R16 T200-40 path verbatim (risk-on
  0.20/name = sleeve 60%, risk-off 0.0889 = 26.7%, idle cash disclosed),
  intents = entry buy / exit sell-full / continuing monthly
  reduce-to-target (C05 convention; engine has no top-up primitive);
  stock symbols run stock semantics (am/pm, T+1, lots, STOCK_FEE_SCHEDULE,
  real stk_limit); meta-overfitting risk disclosed in the prereg sec 0;
- stop clauses: any arm dev_pass AND >=10% -> STOP and report (val is a
  separate user decision); pass but <10% -> bookkeep and report; both
  eliminated -> F-line paused per prereg sec 6.  val stays unconsumed.

Deterministic; no RNG.  Spec seams are NOT silently resolved: they are
recorded and reported.  Budget 1200 s wall (prereg sec 7).
"""
...
PREREG_MD = ROOT / "docs/research/exp-20260919-factor-round-f3r2-prereg.md"
...
F3R2_CONFIG = ROOT / "configs/experiments/f3r2-hybrid-chassis.json"
...
ENGINE_SHA_EXPECT_FULL = ("84a2443ac28fe1b87e4a18f988fd38cc67cf706d5778e262"
                          "f11465147234082c")
...
# config table (F3R2 prereg sec 2/3): chassis legs = R3-06 verbatim
# (sh.511010 15% + sh.518880 25%); F3R2-A0 = R3-06 verbatim replica used
...
CFGS = {
    "F3R2-A0": {"sleeve": None, "legs": dict(CHASSIS_LEGS),
                "rotation": {"universe": UNI6, "top_n": 3, "weight": 0.20,
                             "lookback_m": 6}},   # = R3-06 verbatim (anchor)
    "F3R2-EW": {"sleeve": "EW", "legs": dict(CHASSIS_LEGS), "rotation": None},
    "F3R2-ICW": {"sleeve": "ICW", "legs": dict(CHASSIS_LEGS), "rotation": None},
}
# F3 stock-sleeve parameters (prereg sec 3, frozen)
SLEEVE_K = 3                 # buffer membership: enter top-3, stay rank<=6
SLEEVE_W_ON = 0.20           # per-name target at e_T = 0.90 (risk-on)
...
assert f3r1_mg["engine_sha256_16"] == ENGINE_SHA_EXPECT_FULL[:16], \
...
# buffer membership (H-47 state machine, K=3, q5 filter OFF -- F3R1 line
# convention): enter from top-3, stay while rank <= 6; seats that cannot be
# filled stay CASH (rule-compliant cash slot)
...
for _arm in COMPOSITE:
    ...
    SLEEVE_LOG[_arm] = []
    for t in sig_days:
        new_members, info = r16.buffer_membership(
            members, RANKED_SLEEVE[_arm][t], {}, SLEEVE_K, False)
        rank_of_m = {s: i + 1 for i, s in enumerate(RANKED_SLEEVE[_arm][t])}
        entries = [s for s in new_members if s not in members]
...
sleeve_consistency = {
    ...
    "note": "buffer_membership K=3 q5-off over F3R1 frozen composites; "
            "w_T = e_T*(0.20/0.90), e_T = R16 T200-40 verbatim"}
check_budget("sleeve")
...
# === 10. F3R2-A0 regression anchor vs v3 R3-06 ==============================
log("== 10. F3R2-A0 regression anchor vs v3 R3-06 ==")
results: dict = {}
...
# F3R2-A0 regression: 5-frame .equals vs the v3 run's R3-06 outputs +
# net_cagr must equal the v3 metric (F3R2-A0 is NOT re-adjudicated)
results["F3R2-A0"] = run_one("F3R2-A0")
r0 = results["F3R2-A0"]["res"]
...
regression = {}
for name, mine in (("intents", FRAMES["F3R2-A0"]), ("fills", r0.fills),
                   ("events", r0.events), ("daily", r0.daily),
                   ("clips_final", r0.clips_final)):
    same = mine.equals(ref[name])
    regression[name] = same
    log(f"  F3R2-A0 {name}.equals(v3 R3-06 {name}) = {same} "
        f"(mine {mine.height} rows / ref {ref[name].height} rows)")
regression["net_cagr_matches_v3_metrics"] = \
    abs(_cagr0 - V3_R306["net_cagr"]) <= 1e-12
log(f"  F3R2-A0 net_cagr {_cagr0:.10f} vs v3 R3-06 "
    f"{V3_R306['net_cagr']:.10f} -> "
    f"{regression['net_cagr_matches_v3_metrics']}")
if not all(regression.values()):
    _fail(f"F3R2-A0 REGRESSION FAILURE: {regression} -- abort before remaining "
          "configs per F3R2 prereg sec 6 (F3R2-A0 != v3 R3-06)")
log("  F3R2-A0 regression anchor PASS (== v3 R3-06, 5/5 frames)")

# === 11. remaining configs ====================================================
log("== 11. remaining configs ==")
for cid in [c for c in CONFIG_IDS if c != "F3R2-A0" and c in FRAMES]:
...
# adjudicated configs = arms only; F3R2-A0 is a regression anchor whose
# verdict stays with the v3 run (not re-adjudicated here)
ARMS = [c for c in CONFIG_IDS if c != "F3R2-A0"]
n_pass = sum(1 for cid in ARMS if cid in gates
             and gates[cid]["dev"]["dev_pass"])
log(f"== gates: {n_pass}/{len(ARMS)} dev_pass (arms only; F3R2-A0 anchor "
...
def sleeve_fill_profile(cid: str) -> dict:
    """F3R2: active-sleeve fill profile (rotation UNI6 for the A0 anchor,
...
base_profile = sleeve_fill_profile("F3R2-A0")
for cid in results:
    if cid == "F3R2-A0":
        continue
    prof = sleeve_fill_profile(cid)
    disclosures[cid]["6_sleeve_vs_A0"] = {
        "f3r2_a0": base_profile, "this": prof,
        "delta_sleeve_fills":
        ...
        "caliber": "active-sleeve displacement vs F3R2-A0 (rotation UNI6 for "
                   "the anchor, F3 stock members for the arms): sleeve "
                   "fills / emitted entries-exits / insufficient-cash orders "
                   "(disclosed, not gated)"}

# --- F3R2 disclosure: sleeve churn / path budget ----------------------------
_sleeve_rows = []
for cid in CONFIG_IDS:
...
if _goal_hit:
    ...
    _STOP_NOTE = (f"no ARM passed all dev gates ({n_pass}/{len(ARMS)}); "
                  "F-line PAUSED per the F3R2 prereg stop clause (F3R3 is a "
...
mg = {
    ...
    "f3r2_config_sha256_16": pins["f3r2_config"]["sha256"][:16],
    "f3r2a0_regression_anchor": {**regression,
    ...
    "no_rejurisdiction": "v2/v3-judged configs (H2-01/03/04, ROT-05, "
                         "R3-01..06) are NOT re-run or re-judged in F3R2; "
                         "F3R2-A0 is a regression anchor only (F3R2 prereg "
                         "sec 2)",
    "benchmark_seam_S3": "family anchor B1m/C05 (T200-40, rate_only) + "
                         "B3prime[T200-40] inherited verbatim; the F3R2 "
    ...
    "spec_seams": {
        ...
        "S2_leg_rotation_collision": "RESOLVED by the frozen v2 spec: leg "
                                     ...
                                     "universe; F3R2-A0 verifies at frame "
...
rep = ["# F3R2 判定（exp-20260919-factor-round-f3r2，F3 股票腿 × R3-06 底盘）", "",
       ...
       f"- F3R2-A0 回归锚：intents/fills/events/daily_equity/clips_final "
       f"与 v3 run R3-06 输出逐帧 polars `.equals()` 全部一致（5/5 PASS），"
       f"判定链有效；R3-06 的 v3 判定（dev 八门全过 +11.06%）不被重判。",
...
for cid in results:
    ...
    if comp:
        comp_txt = (
            f"；vs F3R2-A0: 腿成交 {comp['delta_sleeve_fills']:+d} 单, "
            f"现金不足作废 {comp['this']['insufficient_cash_orders']} 次"
            f"（锚 {comp['f3r2_a0']['insufficient_cash_orders']} 次）")
...
rep += ["", "### 159934 冻结披露", "",
        ...
        f"- 引擎 stats：intents {results['F3R2-A0']['res'].stats['intent_layer']['n_intents']}"
        f"，orders {results['F3R2-A0']['res'].stats['intent_layer']['orders_generated']}"
        f"，filled {results['F3R2-A0']['res'].stats['intent_layer']['stopped_filled']}。",
        "", "## 四、运行身份", "",
        ...
        f"- 试验计账：本轮计 3 项（F3R2-A0 锚 + EW/ICW 两臂），策略线 "
        "264→267；因子线 FT-01=120 不变（如台账有异以台账为准）。",
        f"- 判定：{mg['stop_note']}"]
rep += ["", "## 五、关键发现与口径说明", "",
        ...
        "- **停止条款（F3R2 预登记 §6）**：任一臂过门且 ≥10% → 停止并呈用户"
        "（val 消费须用户明确批准）；过门但 <10% → 记账报告；两臂全灭 → "
        "F 线暂停呈用户。val 保持零接触。",
        "- **元过拟合披露（预登记 §0）**：本轮设计受 F3R1 门 4/5/8 死因与 v4 "
        "轴 A 证据启发；dev 读数为第 10 次重用的探索性结果，不构成样本外声称。",
...
manifest = {
    ...
    "command": [sys.executable,
                f"artifacts/runs/{RUN_DIR.name}/scripts/runner_f3r2.py"],
    "random_seed": None,
    ...
    "sleeve_consistency": sleeve_consistency,
    "f3r2a0_regression": regression,
    "leg_struct_check": leg_struct_check,
    "spec_seams": mg["spec_seams"],
    "benchmark_seam_S3": mg["benchmark_seam_S3"],
    "dividend_events_built": dividend_events.height,
    "trial_accounting": {"this_round": "3 (F3R2-A0 anchor + EW + ICW)",
                         "strategy_line": "264->267; factor line FT-01=120 "
                                          "unchanged"},
