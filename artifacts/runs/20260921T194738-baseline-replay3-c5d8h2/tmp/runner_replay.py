# -*- coding: utf-8 -*-
"""Baseline-rebuild replay: 500k stock+cash, five-layer attribution
...
Anchored to the CURRENT HEAD engine sha (the archived pin f3650f84..
matches no git version -- see the audit); this run records its own
engine sha. Stocks only: no ETF leg, no defensive benchmark, cash earns
nothing (registered convention).

Replay v3 (post-review): the single-name halving is the reworked
absolute-event semantics (src/quant/research/single_name_rules.py, whole-
position frozen target + frozen source), fixing the two regressions the
review found in the first Q1 fix (multi-clip oversell; K=3 streak reset).
Also adds: seat_exit/account_trim trigger accounting, event-merged fill
attribution, per-row exit_reason logs, exposure stats and the
investable-start (first fill) return caliber, full module pins.
"""
...
log(f"baseline replay; window={DEV_START}..{DEV_END}; "
...
for name, src, gcfg, snr_on in CURVES:
    ...
    log(f"  {name}: {wall:.1f}s; injected {il.get('dynamic_injected')} "
        ...
        f"filled {il.get('stopped_filled')}; warnings {len(res.warnings)}")
...
def _tally_reasons(reasons: list[dict]) -> dict:
    ...
    (symbol, source, reason): a halving/exit re-issued across decision
    points shares its source and is ONE trigger, not many (audit C3).
    Whole-exit re-issues carry a per-day source, so one stop/protection
    exit spanning several days can still count more than once -- the
    completed-exit side below is the authoritative count."""
    out: dict[str, int] = {}
...
def _tally_filled_reasons(reasons: list[dict], fills) -> dict:
    ...
    halving/exit trigger day or the seat's signal day); fills are
    attributed to the newest reason event at/before the fill's decision
    point and merged per (symbol, source, reason)."""
