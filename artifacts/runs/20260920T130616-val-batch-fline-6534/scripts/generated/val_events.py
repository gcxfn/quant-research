...
# window: see the parametrization header
H_MAIN = 20
H_SENS = (5, 10)
...
VOCAB15 = ["demand_up", "demand_down", "price_up", "price_down", "orders",
           ...
           "other"]
# YEARS_SEG: see the parametrization header
...
(_TMPDIR / f"t1_batch_sha256_{BUILD_TAG}.txt").write_text(
    "\n".join(listing_lines) + "\n", encoding="utf-8", newline="\n")
pins["t1_batch_files"] = {
    "n_files": len(batch_files),
    "listing": str(_TMPDIR / f"t1_batch_sha256_{BUILD_TAG}.txt"),
    "listing_sha256": sha256_file(_TMPDIR / f"t1_batch_sha256_{BUILD_TAG}.txt")}
MANIFEST["pins"] = pins
write_manifest()
...
merged.write_parquet(_TMPDIR / f"annotations_merged_{BUILD_TAG}.parquet")
MANIFEST["pins"]["annotations_merged"] = {
    "path": str(_TMPDIR / f"annotations_merged_{BUILD_TAG}.parquet"),
    "sha256": sha256_file(_TMPDIR / f"annotations_merged_{BUILD_TAG}.parquet")}
...
if chain_max_date > date(2024, 12, 31):
    stop(f"冻结区违例: 收益链日期 {chain_max_date}")
...
drift = (ana.group_by("type")
         ...
         .sort("type"))
drift.write_csv(_OUTDIR / "type_drift.csv")
...
(_OUTDIR / "anova_main.json").write_text(
...
diag.write_csv(_OUTDIR / "category_diagnostics.csv")
...
if _WINDOW == "val":
    ...
    print(f"[val] {n_no_fwd} pool-passed events dropped for lack of a "
          ...
          f"{_wend_max}")
ev_out.write_parquet(_OUTDIR / f"events_{BUILD_TAG}.parquet")
(_OUTDIR / "pool_filter_counts.json").write_text(
...
_sr = _TMPDIR / f"stop_report_events_{BUILD_TAG}.md"
if _sr.exists():
    _sr.unlink()
