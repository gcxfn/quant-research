#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-65 专用：把 6 段逐行结构化判断(batch_065_judge_partN.jsonl)与 row_index 元信息
程序化合并为 batch_065_partN.jsonl（全字段），并做合并前预校验：
...
文件名 batch_024→batch_065，校验逻辑逐行保留（含冻结区断言）。

用法: python merge_batch28.py --repo-root D:/量化
产物: work/batch_065_part1..6.jsonl、data/features/fcst-reason-struct-full-20260918/batch_065.jsonl
"""
import argparse
import csv
import glob
...
def main():
    ...
    fails = []
    merged = []
    part_specs = [(1, 100), (2, 67)]  # tail batch: 167 rows total
    for part, part_len in part_specs:
        jpath = os.path.join(work, f"batch_065_judge_part{part}.jsonl")
        with open(jpath, encoding="utf-8") as f:
            judges = [json.loads(x) for x in f.read().splitlines() if x.strip()]
        expect_lo = 38400 + (part - 1) * 100
        if [j["ordinal"] for j in judges] != list(range(expect_lo, expect_lo + part_len)):
        ...
        with open(os.path.join(work, f"batch_065_part{part}.jsonl"), "w",
    ...
    if fails:
        ...
        for x in fails:
            print("FAIL:", x)
        raise SystemExit(1)
    out = os.path.join(features_dir, "batch_065.jsonl")
    with open(out, "w", encoding="utf-8", newline="\n") as f:
        f.write("\n".join(merged) + "\n")
    print(json.dumps({
        "merged_rows": len(merged),
        "part_files": [f"batch_065_part{i}.jsonl" for i in (1, 2)],
        "output": os.path.relpath(out, root).replace("\\", "/"),
        "precheck": "OK (167/167, quotes verbatim, vocab closed, secondary!=primary, frozen-zone clear)",
