...
if "bond_ladder_monotonicity" in disclosures:
    ...
    rep += [("- 轨迹若非单调 = 结构敏感而非线性机会成本；R3-A0 = 0.25 基点" + ("（val 阶段未运行 R3-A0）" if _WINDOW == "val" else "。")),
...
manifest = {
    ...
    "command": [sys.executable,
                f"artifacts/runs/{RUN_DIR.name}/scripts/runner_val_batch_hybrid.py"],
    ...
    "engine": {"path": str(ENGINE_PY),
               ...
               "version": "v1.4.1 (pin a01cb29c; zones path not used; non-zones path byte-identical to v1.3 84a2443a, anchored by this run's dev arms + the C05 port)"},
    "pins": pins,
