#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-10 标注辅助 v2：生成"ordinal|REASON"单行紧凑视图（控制标注上下文预算）。
...
- 元信息（ts_code/end_date/ann_date/update_flag/type）不进视图：merge 脚本从
  row_index 程序化合入；本批 type 恒为 略减（row_index 已程序确认）。

用法: python extract_cues.py --repo-root D:/量化 [--batch 10]
产物: work/batch_010_cues.txt
"""
import argparse
import csv
import glob
