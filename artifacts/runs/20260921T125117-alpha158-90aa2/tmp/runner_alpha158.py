# -*- coding: utf-8 -*-
"""Alpha158 step-1 (full-set cross-sectional eval) + step-2 (LightGBM
rolling rank-IC). Prereg: docs/research/exp-20260921-alpha158-prereg.md.

Loads the baostock panel once, computes 158 features per (symbol, signal
day) over the R16 executable pool, evaluates all 158 (IC/t/monotonic/
coverage, full distribution reported), then trains frozen-hparam LightGBM
models rolling (train on sections <= t-1, predict t) and reports monthly
Rank IC. Engine replay (step 2 tail) runs only if the IC branch condition
holds (AND-logic short-circuit per prereg).
"""
...
# --- load panel once, per-symbol arrays ------------------------------------
...
# --- features per (signal day, pooled symbol) ------------------------------
FEATURES: dict[str, dict[str, dict]] = {t: {} for t in sig}   # t -> sym -> f
...
# --- forward returns (signal-month close-to-close) --------------------------
...
for fi, name in enumerate(names):
    ...
    for t in sig[:-1]:
        ...
        if ic is not None:
            ics.append(ic)
        # monotonicity: top-minus-bottom half means
        med = np.median(x)
...
log(f"step1: {ok.height} features evaluated; |t|>=2: {strong.height}; "
    f"top ic_mean {ok['ic_mean'][0]:.4f} ({ok['feature'][0]}), "
    f"bottom {ok['ic_mean'][-1]:.4f}")

# --- step 2: LightGBM rolling ----------------------------------------------
...
for t in sig[:-1]:
    ...
    if len(keep) < 100:
        continue
    m = np.array(mat)
    # cross-sectional rank-normalise each feature (0..1), NaN->0.5 median
    for k_ in range(m.shape[1]):
        col = m[:, k_]
        order = np.argsort(np.argsort(col))
        m[:, k_] = order / max(len(col) - 1, 1)
    X[t] = {"syms": np.array(keep), "x": m,
            "y": np.array([FWD[t][s_] for s_ in keep])}
ordered = sorted(X)
log(f"step2: {len(ordered)} monthly sections "
    f"({ordered[0]}..{ordered[-1]}), dim {len(names)}")

PARAMS = dict(objective="regression", n_estimators=300, learning_rate=0.05,
...
for i in range(1, len(ordered)):
    ...
    yt = np.concatenate([X[u]["y"] for u in train_ts])
    model = lgb.LGBMRegressor(**PARAMS)
    model.fit(xt, yt)
    pred = model.predict(X[t]["x"])
    importances += model.feature_importances_
    n_models += 1
    ic = spearman(pred, X[t]["y"])
    if ic is not None:
        ic_series.append({"t": str(t), "rank_ic": ic})
    PRED[t] = {str(s_): float(p) for s_, p in zip(X[t]["syms"], pred)}
    if n_models % 12 == 0:
...
# branch condition part 1 (IC gate)
branch_ic = (mean_ic > 0.03 and t_ic_ml >= 2)
manifest = {
    "run_id": RUN_DIR.name, "experiment_id": "exp-20260921-alpha158",
    "status": "completed",
    "started_at_utc": datetime.now(timezone.utc).isoformat(),
    "step1": {"features_evaluated": ok.height,
              "abs_t_ge_2": strong.height,
              "top": ok.head(10).to_dicts()},
    "step2": {"models": n_models, "ic_months": len(ics),
              "rank_ic_mean": mean_ic, "rank_ic_t": t_ic_ml,
              "by_year": by_year, "top_features": top_feats,
              "ic_gate_passed": branch_ic},
    "branch": ("IC gate " + ("PASSED -- proceed to engine replay"
                             if branch_ic else
                             "FAILED -- daily-OHLCV route closes per prereg "
                             "step-3 branch"))}
(RUN_DIR / "manifest.json").write_text(
    json.dumps(manifest, ensure_ascii=False, indent=1, default=str),
    encoding="utf-8")
log(f"== branch: {manifest['branch']} ==")
LOG.close()
