#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""wave-14 标注辅助：生成"ordinal|REASON"单行紧凑视图（控制标注上下文预算）。

沿自 wave-13 extract_cues.py，仅改 batch 默认 13→14，产物 work/batch_014_cues.txt；
行前缀查 row_index 后简化为"ordinal|REASON"（批 14 区段 type 分布 600 行全部为
"略增"，字段冗余，故不含 type；supports_direction 按改善组=略增判定）。
...
用法: python extract_cues.py --repo-root D:/量化 [--batch 14]
产物: work/batch_014_cues.txt
"""
import argparse
import csv
import glob
