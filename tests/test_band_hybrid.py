...
@pytest.mark.parametrize("decision_session,live_day,live_session", [
    ("pm", date(2024, 1, 3), "am"),
    ("am", date(2024, 1, 2), "pm"),
])
def test_halfday_clock_etf_routes_next_session(decision_session, live_day, live_session):
    days = [date(2024, 1, 2), date(2024, 1, 3)]
    daily = mixed_daily(ETF, [(d, 2, 2.1, 1.9, 2, 2) for d in days])
    half = half_bars(ETF, [(d, s, 2, 2.1, 1.9, 2)
                           for d in days for s in ("am", "pm")])
    intents = intents_frame([it(ETF, "buy", days[0], decision_session,
                                days[0], target=10000)])
    kwargs = {"symbol_meta": {ETF: {"asset_class": "etf", "band": .1,
                                     "t_plus": 0}}, "execution_clock": "halfday"}
    result = run_h(intents, daily, half, stock_limits(daily), **kwargs)
    assert result.stats["execution_clock"] == "halfday"
    assert result.stats["etf_leg"]["routing"].startswith("halfday:")
    assert result.fills.height == 1
    assert result.fills["date"][0] == live_day
    assert result.fills["session"][0] == live_session
    with pytest.raises(BandContractError, match="requires ETF am and pm bars"):
        run_h(intents, daily, half.filter(pl.col("session") == "pm"),
              stock_limits(daily), **kwargs)


def test_halfday_morning_equity_uses_am_close_and_ignores_pm_future():
    other = "sh.510050"
    d1, d2 = date(2024, 1, 2), date(2024, 1, 3)
    outcomes = []
    for future_close in (2.05, 2.15):
        daily = pl.concat([
            mixed_daily(ETF, [(d1, 2, 2.01, 1.99, 2, 2),
                              (d2, 2, 2.2, 1.99, future_close, 2)]),
            mixed_daily(other, [(d, 2, 2.01, 1.99, 2, 2) for d in (d1, d2)])])
        half = pl.concat([
            half_bars(ETF, [(d1, s, 2, 2.01, 1.99, 2) for s in ("am", "pm")]
                      + [(d2, "am", 2, 2.2, 1.99, 2.2),
                         (d2, "pm", 2.2, 2.2, 2, future_close)]),
            half_bars(other, [(d, s, 2, 2.01, 1.99, 2)
                             for d in (d1, d2) for s in ("am", "pm")])])
        meta = {s: {"asset_class": "etf", "band": .1, "t_plus": 0}
                for s in (ETF, other)}
        result = run_h(intents_frame([
            it(ETF, "buy", d1, "pm", d1, target=20000),
            it(other, "buy", d2, "am", d2, weight=.2)]),
            daily, half, stock_limits(daily), symbol_meta=meta,
            execution_clock="halfday")
        bought = result.fills.filter(pl.col("symbol") == other)
        assert bought.height == 1
        # 20,000 invested at 2, 5 commission, then am close 2.2:
        # equity=201,995; 20% / 2 rounded down to 100-share lots=20,100.
        assert bought["shares"][0] == 20100
        outcomes.append(bought)
    assert outcomes[0].equals(outcomes[1])


def test_halfday_normal_intent_expires_after_one_decision_point():
    d1, d2, d3 = date(2024, 1, 2), date(2024, 1, 3), date(2024, 1, 4)
    daily = mixed_daily(ETF, [(d, 2, 2.01, 1.99, 2, 2)
                              for d in (d1, d2, d3)])
    half = half_bars(ETF, [(d1, s, 2, 2.01, 1.99, 2) for s in ("am", "pm")]
                     + [(d2, "am", 2.01, 2.02, 2.00, 2.01),
                        (d2, "pm", 2.01, 2.02, 2.00, 2.01),
                        (d3, "am", 2.01, 2.02, 2.00, 2.01),
                        (d3, "pm", 2.01, 2.02, 2.00, 2.01)])
    # A narrow buy band is never crossed, so a legacy re-hang would be visible.
    result = run_h(intents_frame([it(ETF, "buy", d1, "pm", d1, target=10000)]),
                   daily, half, stock_limits(daily),
                   symbol_meta={ETF: {"asset_class": "etf", "band": .001,
                                      "t_plus": 0}}, execution_clock="halfday")
    assert result.fills.height == 0
    assert result.stats["intent_layer"]["orders_generated"] == 1
    assert result.stats["intent_layer"]["terminated_expired_halfday"] == 1
